...
const outputPath = path.join(root, 'docs/slides/appium-intro-slides.pdf')
const SLIDES_MARKER = 'testflow-appium'
...
async function waitForSlidesServer(url, attempts = 30) {
  ...
  throw new Error(
    `Appium slides were not served at ${url}. ` +
...
async function main() {
  ...
  try {
    ...
    console.log(`Verified Appium slides at ${slidesUrl}`)
